...
func scheduleForLaterToday(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID) {
	...
	message := ahasend.CreateMessageRequest{
		From: ahasend.SenderAddress{
			...
			Name:  ahasend.PtrString("Reminder Service"),
		},
		...
		Schedule: &ahasend.MessageSchedule{
...
func scheduleWeeklyNewsletter(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID) {
	...
	message := ahasend.CreateMessageRequest{
		From: ahasend.SenderAddress{
			...
			Name:  ahasend.PtrString("Weekly Newsletter"),
		},
		Recipients: []ahasend.Recipient{
...
func scheduleBirthdayEmails(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID) {
	...
	for _, user := range birthdays {
		...
		message := ahasend.CreateMessageRequest{
			From: ahasend.SenderAddress{
				...
				Name:  ahasend.PtrString("Birthday Team"),
			},
			Recipients: []ahasend.Recipient{
				{
					Email: user.email,
					Name:  ahasend.PtrString(user.name),
				},
			},
...
func scheduleEmailCampaign(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID) {
	...
	message := ahasend.CreateMessageRequest{
		From: ahasend.SenderAddress{
			...
			Name:  ahasend.PtrString("Product Team"),
		},
		Recipients: []ahasend.Recipient{
			{Email: "customer1@example.com", Name: ahasend.PtrString("Customer 1")},
			{Email: "customer2@example.com", Name: ahasend.PtrString("Customer 2")},
			{Email: "customer3@example.com", Name: ahasend.PtrString("Customer 3")},
		},
...
func cancelScheduledEmail(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID) {
	...
	response, httpResp, err := client.MessagesAPI.
		CancelMessage(ctx, accountID, messageID).
		Execute()

	if err != nil {
		fmt.Printf("❌ Could not cancel: %v\n", err)
		fmt.Println("   Message may have already been sent or doesn't exist")
	} else {
