...
func listWebhooks(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID) {
	response, httpResp, err := client.WebhooksAPI.
		GetWebhooks(ctx, accountID).
		Limit(10).
		Execute()
	...
	if httpResp.StatusCode >= 200 && httpResp.StatusCode < 300 {
		if response.Data != nil && len(response.Data) > 0 {
			...
			for _, webhook := range response.Data {
				...
				var triggers []string
				if webhook.OnReception != nil && *webhook.OnReception {
					triggers = append(triggers, "reception")
				}
				if webhook.OnDelivered != nil && *webhook.OnDelivered {
					triggers = append(triggers, "delivered")
				}
				if webhook.OnBounced != nil && *webhook.OnBounced {
					triggers = append(triggers, "bounced")
				}
				if webhook.OnOpened != nil && *webhook.OnOpened {
					triggers = append(triggers, "opened")
				}
				if webhook.OnClicked != nil && *webhook.OnClicked {
...
func createWebhook(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID) string {
	request := ahasend.CreateWebhookRequest{
		Name:        "Email Event Webhook",
		Url:         "https://your-domain.com/webhooks/ahasend",
		Enabled:     ahasend.PtrBool(true),
		OnReception: ahasend.PtrBool(true),
		OnDelivered: ahasend.PtrBool(true),
		OnBounced:   ahasend.PtrBool(true),
		OnOpened:    ahasend.PtrBool(true),
		OnClicked:   ahasend.PtrBool(true),
	}

	response, httpResp, err := client.WebhooksAPI.
		CreateWebhook(ctx, accountID).
...
func getWebhookDetails(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID, webhookID string) {
	...
	response, httpResp, err := client.WebhooksAPI.
		GetWebhook(ctx, accountID, webhookUUID).
		Execute()
	...
	if httpResp.StatusCode >= 200 && httpResp.StatusCode < 300 && response != nil {
		...
		if webhook.OnReception != nil {
			fmt.Printf("    On Reception: %v\n", *webhook.OnReception)
		}
		if webhook.OnDelivered != nil {
			fmt.Printf("    On Delivered: %v\n", *webhook.OnDelivered)
		}
		if webhook.OnBounced != nil {
			fmt.Printf("    On Bounced: %v\n", *webhook.OnBounced)
		}
		if webhook.OnOpened != nil {
			fmt.Printf("    On Opened: %v\n", *webhook.OnOpened)
		}
		if webhook.OnClicked != nil {
			fmt.Printf("    On Clicked: %v\n", *webhook.OnClicked)
		}
		if webhook.OnFailed != nil {
			fmt.Printf("    On Failed: %v\n", *webhook.OnFailed)
		}
		if webhook.OnSuppressed != nil {
			fmt.Printf("    On Suppressed: %v\n", *webhook.OnSuppressed)
		}
	}
}

func updateWebhook(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID, webhookID string) {
	webhookUUID, err := uuid.Parse(webhookID)
	if err != nil {
		log.Printf("Invalid webhook ID: %v", err)
		return
	...
	request := ahasend.UpdateWebhookRequest{
		Name:                 ahasend.PtrString("Updated Email Event Webhook"),
		Enabled:              ahasend.PtrBool(true),
		OnReception:          ahasend.PtrBool(true),
		OnDelivered:          ahasend.PtrBool(true),
		OnBounced:            ahasend.PtrBool(true),
		OnOpened:             ahasend.PtrBool(true),
		OnClicked:            ahasend.PtrBool(true),
		OnSuppressionCreated: ahasend.PtrBool(true), // Added new event
	}

	response, httpResp, err := client.WebhooksAPI.
		UpdateWebhook(ctx, accountID, webhookUUID).
	...
	if httpResp.StatusCode >= 200 && httpResp.StatusCode < 300 {
		...
		if response != nil {
			...
			if response.OnReception != nil {
				fmt.Printf("    On Reception: %v\n", *response.OnReception)
			}
			if response.OnDelivered != nil {
				fmt.Printf("    On Delivered: %v\n", *response.OnDelivered)
			}
			if response.OnBounced != nil {
				fmt.Printf("    On Bounced: %v\n", *response.OnBounced)
			}
			if response.OnOpened != nil {
				fmt.Printf("    On Opened: %v\n", *response.OnOpened)
			}
			if response.OnClicked != nil {
				fmt.Printf("    On Clicked: %v\n", *response.OnClicked)
			}
			if response.OnSuppressionCreated != nil {
...
func deleteWebhook(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID, webhookID string) {
	webhookUUID, err := uuid.Parse(webhookID)
	if err != nil {
		log.Printf("Invalid webhook ID: %v", err)
		return
	...
	response, httpResp, err := client.WebhooksAPI.
		DeleteWebhook(ctx, accountID, webhookUUID).
		Execute()
