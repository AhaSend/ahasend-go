...
func main() {
	...
	// You can provide manual idempotency keys via the IdempotencyKey() method.
...
func demonstrateAutomaticIdempotency(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID) {
	...
	message := ahasend.CreateMessageRequest{
		...
		TextContent: ahasend.PtrString("This email has automatic idempotency protection."),
	}

	// First send
	fmt.Print("Sending email (1st attempt)... ")
	response1, httpResp1, err := client.MessagesAPI.
...
func demonstrateManualIdempotency(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID) {
	...
	message := ahasend.CreateMessageRequest{
		...
		TextContent: ahasend.PtrString("Your order has been confirmed."),
	}
	...
	response, httpResp, err := client.MessagesAPI.
		CreateMessage(ctx, accountID).
		CreateMessageRequest(message).
		IdempotencyKey(idempotencyKey). // Set manual key
		Execute()
	...
	response2, httpResp2, err2 := client.MessagesAPI.
		CreateMessage(ctx, accountID).
		CreateMessageRequest(message).
...
func demonstrateDuplicatePrevention(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID) {
		...
		Recipients: []ahasend.Recipient{
			{
				...
				Name:  ahasend.PtrString("New User"),
			},
		},
	...
	for i := 1; i <= attempts; i++ {
		...
		_, httpResp, err := client.MessagesAPI.
			CreateMessage(ctx, accountID).
			CreateMessageRequest(message).
...
func demonstrateIdempotencyWithRetries(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID) {
	fmt.Println("Implementing safe retry logic with idempotency")
	fmt.Println("")

	// Create a standard client for retry example
	configuration := ahasend.NewConfiguration()
	retryClient := ahasend.NewAPIClient(configuration)
...
func demonstrateTransactionSafeSending(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID) {
	...
	message := ahasend.CreateMessageRequest{
		From: ahasend.SenderAddress{
			...
			Name:  ahasend.PtrString("Billing Department"),
		},
		Recipients: []ahasend.Recipient{
			{
				Email: "customer@example.com",
				Name:  ahasend.PtrString("Valued Customer"),
			},
		},
		Subject: fmt.Sprintf("Payment Confirmation - %s", transactionID),
		HtmlContent: ahasend.PtrString(fmt.Sprintf(`
