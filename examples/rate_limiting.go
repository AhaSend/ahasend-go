...
func demonstrateDefaultRateLimiting(client *ahasend.APIClient) {
...
func demonstrateCustomRateLimiting(client *ahasend.APIClient) {
...
func demonstrateBurstHandling(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID) {
	...
	for i := 1; i <= 20; i++ {
		...
		go func(num int) {
			...
			message := ahasend.CreateMessageRequest{
				...
				TextContent: ahasend.PtrString("Rate limiting test"),
			}
...
func demonstratePerEndpointRateLimiting(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID) {
	// Different endpoints have different rate limits
	fmt.Println("Different endpoints have different rate limits:")
	fmt.Println("")

	// Fast endpoint - Messages
	fmt.Print("Sending 3 messages (high rate limit)... ")
	start := time.Now()
	for i := 0; i < 3; i++ {
		client.MessagesAPI.GetMessages(ctx, accountID).Limit(1).Execute()
	}
	...
	start = time.Now()
	for i := 0; i < 3; i++ {
		client.StatisticsAPI.GetDeliverabilityStatistics(ctx, accountID).Execute()
	}
	fmt.Printf("Done in %v\n", time.Since(start))
...
func monitorRateLimitStatus(client *ahasend.APIClient) {
	// Monitor current rate limit status
	endpoints := []ahasend.EndpointType{
		ahasend.GeneralAPI,
		ahasend.SendMessageAPI,
		ahasend.StatisticsAPI,
	}
...
func demonstrateGracefulDegradation(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID) {
	...
	if status.TokensAvailable < 10 {
		...
		client.EnableRateLimit(ahasend.StatisticsAPI, false)
		fmt.Println("   • Disabled rate limiting for statistics (use with caution!)")

		// Re-enable after recovery
		time.Sleep(2 * time.Second)
		client.EnableRateLimit(ahasend.StatisticsAPI, true)
