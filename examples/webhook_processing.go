...
func main() {
	...
	verifier, err := ahasend.NewWebhookVerifier(webhookSecret)
	if err != nil {
...
func webhookHandler(verifier *ahasend.WebhookVerifier) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			switch err {
			case ahasend.ErrMissingHeaders, ahasend.ErrInvalidSignature, ahasend.ErrExpiredTimestamp:
				http.Error(w, "Unauthorized", http.StatusUnauthorized)
			case ahasend.ErrInvalidPayload, ahasend.ErrUnknownEventType:
...
func handleMessageReception(event *ahasend.MessageReceptionEvent) error {
...
func handleMessageDelivered(event *ahasend.MessageDeliveredEvent) error {
...
func handleMessageTransientError(event *ahasend.MessageTransientErrorEvent) error {
...
func handleMessageFailed(event *ahasend.MessageFailedEvent) error {
...
func handleMessageBounced(event *ahasend.MessageBouncedEvent) error {
...
func handleMessageSuppressed(event *ahasend.MessageSuppressedEvent) error {
...
func handleMessageOpened(event *ahasend.MessageOpenedEvent) error {
...
func handleMessageClicked(event *ahasend.MessageClickedEvent) error {
...
func handleSuppressionCreated(event *ahasend.SuppressionCreatedEvent) error {
...
func handleDomainDNSError(event *ahasend.DomainDNSErrorEvent) error {
...
func handleRouteMessage(event *ahasend.RouteMessageEvent) error {
...
func exampleDatabaseIntegration(event ahasend.WebhookEvent) {
	...
	if ahasend.IsSuppressionEvent(event) {
		if suppEvent, ok := event.(*ahasend.SuppressionCreatedEvent); ok {
...
func exampleAdvancedWebhookHandler(verifier *ahasend.WebhookVerifier) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			switch err {
			case ahasend.ErrExpiredTimestamp:
			...
			case ahasend.ErrInvalidSignature, ahasend.ErrMissingHeaders:
				http.Error(w, "Unauthorized", http.StatusUnauthorized)
				return
			default:
				http.Error(w, "Bad Request", http.StatusBadRequest)
