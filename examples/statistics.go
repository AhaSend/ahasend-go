...
func getDeliverabilityStats(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID, startDate, endDate time.Time) {
	response, httpResp, err := client.StatisticsAPI.
	...
	if httpResp.StatusCode >= 200 && httpResp.StatusCode < 300 && response.Data != nil && len(response.Data) > 0 {
		var totalSent, totalDelivered, totalBounced, totalFailed, totalSuppressed int32

		for _, stat := range response.Data {
			if stat.Sent != nil {
				totalSent += *stat.Sent
			}
			if stat.Delivered != nil {
				totalDelivered += *stat.Delivered
			}
			if stat.Bounced != nil {
				totalBounced += *stat.Bounced
			}
			if stat.Failed != nil {
				totalFailed += *stat.Failed
			}
			if stat.Suppressed != nil {
				totalSuppressed += *stat.Suppressed
			}
		}
		...
		for i := len(response.Data) - dayCount; i < len(response.Data); i++ {
			...
			fmt.Printf("    %s: Sent=%d, Delivered=%d, Bounced=%d\n",
				stat.TimeBucket.Format("Jan 02"),
				getInt32Value(stat.Sent),
				getInt32Value(stat.Delivered),
				getInt32Value(stat.Bounced))
		}
	} else {
...
func getBounceStats(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID, startDate, endDate time.Time) {
	response, httpResp, err := client.StatisticsAPI.
		GetBounceStatistics(ctx, accountID).
		FromDate(startDate).
		ToDate(endDate).
		Execute()
	...
	if httpResp.StatusCode >= 200 && httpResp.StatusCode < 300 && response.Data != nil && len(response.Data) > 0 {
		var totalBounces int32
		bouncesByClassification := make(map[string]int32)

		for _, stat := range response.Data {
			totalBounces += stat.Count
			bouncesByClassification[stat.Classification] += stat.Count
		}
...
func getDeliveryTimeStats(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID, startDate, endDate time.Time) {
	response, httpResp, err := client.StatisticsAPI.
		GetDeliveryTimeStatistics(ctx, accountID).
		FromDate(startDate).
		ToDate(endDate).
		Execute()
	...
	if httpResp.StatusCode >= 200 && httpResp.StatusCode < 300 && response.Data != nil && len(response.Data) > 0 {
		...
		for _, stat := range response.Data {
			if stat.Count > 0 {
				...
				avgDeliveryTime += float64(stat.Count) * float64(stat.AvgDeliveryTime)
			}
		}
		...
		for _, stat := range response.Data {
			if stat.Count > 0 {
				percentage := calculatePercentage(stat.Count, totalMessages)
				domain := "All domains"
				if stat.RecipientDomain != nil {
					domain = *stat.RecipientDomain
				}
				fmt.Printf("    %s (%s): %d messages, avg %.2fs (%.1f%%)\n",
					stat.TimeBucket.Format("Jan 02"),
					domain,
					stat.Count,
...
func getRecentMessages(ctx context.Context, client *ahasend.APIClient, accountID uuid.UUID) {
...
func calculatePercentage(value, total int32) float64 {
	if total == 0 {
		return 0
	}
