...
func NewTestClient(config TestClientConfig) *ahasend.APIClient {
...
func CreateTestAuthContext(apiKey string) context.Context {
	...
	return context.WithValue(context.Background(), ahasend.ContextAccessToken, apiKey)
}
...
func (td *IntegrationTestData) CreateMessageRequest(overrides ...func(*ahasend.CreateMessageRequest)) ahasend.CreateMessageRequest {
	req := ahasend.CreateMessageRequest{
		...
		Subject:     "Test Email",
		HtmlContent: ahasend.PtrString("<p>This is a test email from integration tests.</p>"),
		TextContent: ahasend.PtrString("This is a test email from integration tests."),
	}
...
func (td *IntegrationTestData) CreateDomainRequest(domain string) ahasend.CreateDomainRequest {
	if domain == "" {
		domain = "test-example.com"
	}

	return ahasend.CreateDomainRequest{
		Domain: domain,
	}
}

// CreateWebhookRequest generates a test webhook creation request
func (td *IntegrationTestData) CreateWebhookRequest() ahasend.CreateWebhookRequest {
	return ahasend.CreateWebhookRequest{
...
func (tr *IntegrationTestRunner) Client() *ahasend.APIClient {
	return tr.client
}
...
func CaptureHTTPRequest(client *ahasend.APIClient) *HTTPRequestCapture {
