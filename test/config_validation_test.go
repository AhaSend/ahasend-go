...
func TestValidateConfiguration(t *testing.T) {
	t.Run("Nil configuration", func(t *testing.T) {
		result := ahasend.ValidateConfiguration(nil)
	...
	t.Run("Invalid rate limit config", func(t *testing.T) {
		cfg := ahasend.NewConfiguration()
		cfg.DefaultGeneralRateLimit = &ahasend.RateLimitConfig{
			RequestsPerSecond: -1,
			BurstCapacity:     0,
		}
		result := ahasend.ValidateConfiguration(cfg)
...
func TestApplyDefaults(t *testing.T) {
	...
	t.Run("Nil config handling", func(t *testing.T) {
		// Should not panic
		ahasend.ApplyDefaults(nil)
	})
}

func TestValidateAndApplyDefaults(t *testing.T) {
	cfg := &ahasend.Configuration{}
	result := ahasend.ValidateAndApplyDefaults(cfg)
...
func TestNewValidatedConfiguration(t *testing.T) {
	cfg, result := ahasend.NewValidatedConfiguration()
...
func TestNewValidatedConfigurationFromEnv(t *testing.T) {
	withEnv(t, "AHASEND_DEBUG", "true", func() {
		cfg, result := ahasend.NewValidatedConfigurationFromEnv()
...
func TestGetConfigurationSummary(t *testing.T) {
	cfg := ahasend.NewConfiguration()
	summary := ahasend.GetConfigurationSummary(cfg)
...
func TestOptimizeForProduction(t *testing.T) {
	...
	ahasend.OptimizeForProduction(cfg)
...
func TestValidationResultMethods(t *testing.T) {
	t.Run("HasErrors and HasWarnings", func(t *testing.T) {
		result := ahasend.ValidationResult{}
		assert.False(t, result.HasErrors())
		assert.False(t, result.HasWarnings())

		result.Errors = append(result.Errors, ahasend.ConfigurationValidationError{
	...
	t.Run("Error formatting", func(t *testing.T) {
		result := ahasend.ValidationResult{
			Errors: []ahasend.ConfigurationValidationError{
...
func TestConfigurationValidationError(t *testing.T) {
	err := ahasend.ConfigurationValidationError{
		Field:   "TestField",
		Value:   "test-value",
