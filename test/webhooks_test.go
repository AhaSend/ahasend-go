...
func TestNewWebhookVerifier(t *testing.T) {
	t.Run("valid secret", func(t *testing.T) {
		verifier, err := ahasend.NewWebhookVerifier(testWebhookSecret)
...
func TestWebhookVerification(t *testing.T) {
	...
	t.Run("invalid signature", func(t *testing.T) {
		...
		err := verifier.Verify([]byte(payload), headers)
		assert.ErrorIs(t, err, ahasend.ErrInvalidSignature)
	})
	...
	t.Run("expired timestamp", func(t *testing.T) {
		...
		err := verifier.Verify([]byte(payload), headers)
		assert.ErrorIs(t, err, ahasend.ErrExpiredTimestamp)
	})
	...
	t.Run("missing headers", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				assert.ErrorIs(t, err, ahasend.ErrMissingHeaders)
			})
		}
	})
...
func TestWebhookParsing(t *testing.T) {
	...
	t.Run("parse message.delivered event", func(t *testing.T) {
		...
		deliveredEvent, ok := event.(*ahasend.MessageDeliveredEvent)
		require.True(t, ok)
	...
	t.Run("parse message.opened event with extra fields", func(t *testing.T) {
		...
		openedEvent, ok := event.(*ahasend.MessageOpenedEvent)
		require.True(t, ok)
	...
	t.Run("parse message.clicked event", func(t *testing.T) {
		...
		clickedEvent, ok := event.(*ahasend.MessageClickedEvent)
		require.True(t, ok)
	...
	t.Run("parse suppression.created event", func(t *testing.T) {
		...
		suppressionEvent, ok := event.(*ahasend.SuppressionCreatedEvent)
		require.True(t, ok)
	...
	t.Run("parse domain.dns_error event", func(t *testing.T) {
		...
		domainEvent, ok := event.(*ahasend.DomainDNSErrorEvent)
		require.True(t, ok)
	...
	t.Run("parse route.message event", func(t *testing.T) {
		...
		routeEvent, ok := event.(*ahasend.RouteMessageEvent)
		require.True(t, ok)
	...
	t.Run("parse unknown event type", func(t *testing.T) {
		...
		event, err := verifier.Parse([]byte(payload), createValidHeaders(payload))
		assert.Error(t, err)
		assert.ErrorIs(t, err, ahasend.ErrUnknownEventType)
		assert.Nil(t, event)
	})
	...
	t.Run("parse invalid JSON", func(t *testing.T) {
		...
		event, err := verifier.Parse([]byte(payload), createValidHeaders(payload))
		assert.Error(t, err)
		assert.ErrorIs(t, err, ahasend.ErrInvalidPayload)
		assert.Nil(t, event)
	})
...
func TestWebhookHelperFunctions(t *testing.T) {
	t.Run("IsMessageEvent", func(t *testing.T) {
		assert.True(t, ahasend.IsMessageEvent(&ahasend.MessageDeliveredEvent{}))
		assert.True(t, ahasend.IsMessageEvent(&ahasend.MessageOpenedEvent{}))
		assert.True(t, ahasend.IsMessageEvent(&ahasend.MessageClickedEvent{}))
		assert.False(t, ahasend.IsMessageEvent(&ahasend.SuppressionCreatedEvent{}))
		assert.False(t, ahasend.IsMessageEvent(&ahasend.DomainDNSErrorEvent{}))
	})

	t.Run("IsSuppressionEvent", func(t *testing.T) {
		assert.True(t, ahasend.IsSuppressionEvent(&ahasend.SuppressionCreatedEvent{}))
		assert.False(t, ahasend.IsSuppressionEvent(&ahasend.MessageDeliveredEvent{}))
	})

	t.Run("IsDomainEvent", func(t *testing.T) {
		assert.True(t, ahasend.IsDomainEvent(&ahasend.DomainDNSErrorEvent{}))
		assert.False(t, ahasend.IsDomainEvent(&ahasend.MessageDeliveredEvent{}))
	})

	t.Run("IsRouteEvent", func(t *testing.T) {
		assert.True(t, ahasend.IsRouteEvent(&ahasend.RouteMessageEvent{}))
		assert.False(t, ahasend.IsRouteEvent(&ahasend.MessageDeliveredEvent{}))
	})

	t.Run("GetMessageEventData", func(t *testing.T) {
		messageData := ahasend.MessageEventData{
		...
		deliveredEvent := &ahasend.MessageDeliveredEvent{Data: messageData}
		data := ahasend.GetMessageEventData(deliveredEvent)
		require.NotNil(t, data)
		assert.Equal(t, "test-account", data.AccountID)
		assert.Equal(t, "test@example.com", data.From)

		// Test with non-message event
		suppressionEvent := &ahasend.SuppressionCreatedEvent{}
		data = ahasend.GetMessageEventData(suppressionEvent)
		assert.Nil(t, data)

		// Test with clicked event (which has different data structure)
		clickedEvent := &ahasend.MessageClickedEvent{}
		data = ahasend.GetMessageEventData(clickedEvent)
		assert.Nil(t, data)
	})
}
