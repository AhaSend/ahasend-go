...
func TestIdempotencyConfig(t *testing.T) {
	config := ahasend.DefaultIdempotencyConfig()
...
func TestIdempotencyKeyBuilder(t *testing.T) {
	...
	builder := ahasend.NewIdempotencyKeyBuilder("base-key")
	...
	autoBuilder := ahasend.NewIdempotencyKeyBuilder("")
...
func TestIdempotencyHelper(t *testing.T) {
	...
	helper := ahasend.NewIdempotencyHelper()
	...
	configWithPrefix := ahasend.IdempotencyConfig{
		AutoGenerate: true,
		KeyPrefix:    "prefix",
	}
	helperWithPrefix := ahasend.NewIdempotencyHelper(configWithPrefix)
	...
	configNoAuto := ahasend.IdempotencyConfig{
		AutoGenerate: false,
		KeyPrefix:    "",
	}
	helperNoAuto := ahasend.NewIdempotencyHelper(configNoAuto)
...
func TestExecuteIdempotent(t *testing.T) {
	...
	customKey := "test-key"
	result, err := ahasend.ExecuteIdempotent(func(key string) (string, error) {
	...
	result2, err := ahasend.ExecuteIdempotent(func(key string) (string, error) {
		assert.NotEmpty(t, key, "Should generate key")
		assert.Len(t, key, 36, "Should be UUID length")
...
func TestAPIClientIdempotencyIntegration(t *testing.T) {
	...
	newConfig := ahasend.IdempotencyConfig{
		AutoGenerate: false,
		KeyPrefix:    "test",
	}
