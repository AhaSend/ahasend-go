...
func TestBasicIntegrationInfrastructure(t *testing.T) {
	...
	t.Run("Authentication context creation", func(t *testing.T) {
		...
		ctx := context.WithValue(context.Background(), ahasend.ContextAccessToken, testAPIKey)

		// Verify the context contains the API key
		value := ctx.Value(ahasend.ContextAccessToken)
	...
	t.Run("Rate limiting configuration", func(t *testing.T) {
		...
		statisticsStatus := client.GetRateLimitStatus(ahasend.StatisticsAPI)
		assert.Equal(t, ahasend.StatisticsAPI, statisticsStatus.EndpointType)
	...
	t.Run("Environment configuration", func(t *testing.T) {
		...
		apiKey := ctx.Value(ahasend.ContextAccessToken)
...
func TestErrorHandlingInfrastructure(t *testing.T) {
	t.Run("API Error types", func(t *testing.T) {
		...
		authErr := &ahasend.APIError{
			Type:       ahasend.ErrorTypeAuthentication,
			StatusCode: 401,
			Message:    "Invalid API key",
		}

		assert.Equal(t, ahasend.ErrorTypeAuthentication, authErr.Type)
		assert.False(t, authErr.IsRetryable())
		assert.Equal(t, 401, authErr.StatusCode)

		rateLimitErr := &ahasend.APIError{
			Type:       ahasend.ErrorTypeRateLimit,
			StatusCode: 429,
			Message:    "Rate limit exceeded",
		}

		assert.Equal(t, ahasend.ErrorTypeRateLimit, rateLimitErr.Type)
	...
	t.Run("Error helpers", func(t *testing.T) {
		apiErr := &ahasend.APIError{Type: ahasend.ErrorTypeValidation}
		netErr := &ahasend.NetworkError{Op: "test", Err: context.Canceled}

		_, isAPIErr1 := ahasend.IsAPIError(apiErr)
		_, isAPIErr2 := ahasend.IsAPIError(netErr)
		assert.True(t, isAPIErr1)
		assert.False(t, isAPIErr2)

		_, isNetErr1 := ahasend.IsNetworkError(netErr)
		_, isNetErr2 := ahasend.IsNetworkError(apiErr)
		assert.True(t, isNetErr1)
		assert.False(t, isNetErr2)

		assert.True(t, ahasend.IsRetryableError(netErr))
		assert.False(t, ahasend.IsRetryableError(apiErr))
	})
}
...
func TestContextCancellationInfrastructure(t *testing.T) {
	t.Run("Context cancellation with rate limiter", func(t *testing.T) {
		...
		rateLimiter := ahasend.NewRateLimiter()
		rateLimiter.SetRateLimit(ahasend.GeneralAPI, 1, 1) // 1 req/s, 1 burst
...
func BenchmarkBasicOperations(b *testing.B) {
	...
	b.Run("ConfigValidation", func(b *testing.B) {
		cfg := ahasend.NewConfiguration()
		for i := 0; i < b.N; i++ {
			_ = ahasend.ValidateConfiguration(cfg)
		}
	})

	b.Run("ContextCreation", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_ = context.WithValue(context.Background(), ahasend.ContextAccessToken, "test-key")
		}
	})
}
