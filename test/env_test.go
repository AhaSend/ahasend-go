...
func TestConfigFromEnv(t *testing.T) {
	t.Run("Default values when no env vars", func(t *testing.T) {
		...
		assert.Equal(t, 3, cfg.MaxRetries, "Should have default max retries")
	...
	t.Run("Load rate limiting configuration", func(t *testing.T) {
		withEnv(t, "AHASEND_ENABLE_RATE_LIMIT", "false", func() {
			withEnv(t, "AHASEND_MAX_RETRIES", "5", func() {
				...
				assert.Equal(t, 5, cfg.MaxRetries, "Should set max retries")
			})
		})
	})
...
func TestContextWithEnvAuth(t *testing.T) {
	t.Run("With API key", func(t *testing.T) {
		withEnv(t, "AHASEND_API_KEY", "test-key", func() {
			...
			authCtx := ahasend.ContextWithEnvAuth(ctx)

			value := authCtx.Value(ahasend.ContextAccessToken)
	...
	t.Run("No timeouts set", func(t *testing.T) {
		withEnv(t, "AHASEND_TIMEOUT", "", func() {
			withEnv(t, "AHASEND_CONNECT_TIMEOUT", "", func() {
				timeout, connectTimeout := ahasend.GetTimeoutFromEnv()
...
func TestLoadEnvIntoConfig(t *testing.T) {
	t.Run("Load into existing config", func(t *testing.T) {
		...
		cfg.MaxRetries = 1 // Start with 1 retry
		...
		withEnv(t, "AHASEND_DEBUG", "false", func() {
			withEnv(t, "AHASEND_MAX_RETRIES", "5", func() {
				...
				assert.Equal(t, 5, cfg.MaxRetries, "Should override max retries")
			})
		})
	})
	...
	for _, tc := range testCases {
		withEnv(t, "AHASEND_MAX_RETRIES", tc.input, func() {
			...
			if tc.expected == nil {
				...
				assert.Equal(t, 3, cfg.MaxRetries, "Should use default for invalid value: %s", tc.input)
			} else if *tc.expected >= 0 {
				assert.Equal(t, *tc.expected, cfg.MaxRetries, "Should parse integer: %s", tc.input)
			} else {
				// Negative values should use default
				assert.Equal(t, 3, cfg.MaxRetries, "Should use default for negative value: %s", tc.input)
			}
		})
	}
}
